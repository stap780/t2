...
class MoyskladApi::UpdateQuantities
  ...
  def call
    Rails.logger.info "MoyskladApi::UpdateQuantities: Starting quantity update at #{Time.zone.now}"

    # Обнуляем все остатки перед обновлением
    Variant.update_all(quantity: 0)
    Rails.logger.info "MoyskladApi::UpdateQuantities: Reset all variant quantities to 0"
    ...
    Rails.logger.info "MoyskladApi::UpdateQuantities: Updated #{updated_count} variants, #{stations_updated} stations updated, #{with_quantity_count} variants with quantity > 0"
    ...
    Rails.logger.error "MoyskladApi::UpdateQuantities: API error - #{e.response.code}: #{e.response.body}"
    ...
    Rails.logger.error "MoyskladApi::UpdateQuantities: Error - #{e.class}: #{e.message}"
  ...
  def fetch_stock_data
    ...
    loop do
      begin
        ...
        case response.code
        when 200
          Rails.logger.info "MoyskladApi::UpdateQuantities: API response 200 - OK"
          return JSON.parse(response.body)
        when 404
          Rails.logger.error "MoyskladApi::UpdateQuantities: API response 404 - Not Found"
          return []
        when 500, 502, 503, 504, 408
          if retry_count < max_retries
            retry_count += 1
            Rails.logger.warn "MoyskladApi::UpdateQuantities: API response #{response.code} - retry #{retry_count}/#{max_retries} in 1 minute"
            sleep(60)
            next
          else
            Rails.logger.error "MoyskladApi::UpdateQuantities: Max retries reached for #{response.code}"
            return []
          end
        when 422
          Rails.logger.error "MoyskladApi::UpdateQuantities: API response 422 - Validation Error"
          error_data = JSON.parse(response.body) rescue {}
          Rails.logger.error "Error details: #{error_data.inspect}"
          return []
        else
          Rails.logger.error "MoyskladApi::UpdateQuantities: Unexpected API response #{response.code}"
          return []
        end
      rescue RestClient::ExceptionWithResponse => e
        if [500, 502, 503, 504, 408].include?(e.response.code) && retry_count < max_retries
          retry_count += 1
          Rails.logger.warn "MoyskladApi::UpdateQuantities: Retry #{retry_count}/#{max_retries} after error #{e.response.code}"
          sleep(60)
          next
        else
          Rails.logger.error "MoyskladApi::UpdateQuantities: API error after retries: #{e.response.code} - #{e.response.body}"
          return []
        end
      rescue StandardError => e
        Rails.logger.error "MoyskladApi::UpdateQuantities: Unexpected error: #{e.class} - #{e.message}"
        return []
      end
    end
  end
  ...
  def update_station_feature(product, station_name)
    ...
    Rails.logger.error "MoyskladApi::UpdateQuantities: Error updating station feature for product #{product.id}: #{e.message}"
    nil
  end
  ...
  def create_email_delivery_and_notify(result, error_class = nil)
    ...
    Rails.logger.error "MoyskladApi::UpdateQuantities: Error creating email delivery: #{e.class} - #{e.message}"
